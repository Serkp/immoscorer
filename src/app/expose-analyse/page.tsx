...
export default function ExposeAnalysePage() {
  ...
  const canAnalyze = exposeText.trim().length >= 50;
  ...
  async function handlePdfUpload(file: File) {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setError(data.error || "Fehler beim Lesen der PDF.");
        setPdfFile(null);
        setPdfFileName("");
        setPdfParsing(false);
        return;
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 md:py-12">
        ...
        <Card>
          <div className="p-5 md:p-6">
            {/* Tab Toggle */}
            <div className="flex items-center gap-1 p-1 rounded-xl mb-5" style={{ background: C.surface }}>
              <button
                onClick={() => setInputMode("text")}
                className="flex-1 flex items-center justify-center gap-2 h-9 rounded-lg text-[13px] font-medium transition-all"
                style={{
                  background: inputMode === "text" ? C.surface3 : "transparent",
                  color: inputMode === "text" ? C.text : C.sub,
                  border: inputMode === "text" ? `1px solid ${C.border}` : "1px solid transparent",
                }}
              >
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 h-9 rounded-lg text-[13px] font-medium transition-all"
                style={{
                  background: inputMode === "pdf" ? C.surface3 : "transparent",
                  color: inputMode === "pdf" ? C.text : C.sub,
                  border: inputMode === "pdf" ? `1px solid ${C.border}` : "1px solid transparent",
                }}
              >
                <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <>
                {!pdfFile && !pdfParsing ? (
                  <div
                    ...
                    style={{
                      background: C.surface,
                      border: `2px dashed ${error ? C.red : C.border}`,
                      minHeight: 240,
                    }}
                    onClick={() => fileInputRef.current?.click()}
                    onDragOver={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = C.accent; }}
                    onDragLeave={(e) => { e.currentTarget.style.borderColor = error ? C.red : C.border; }}
                    onDrop={handleDrop}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.accent; e.currentTarget.style.background = C.surface2; }}
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = error ? C.red : C.border; e.currentTarget.style.background = C.surface; }}
                  >
                    <input
                      ref={fileInputRef}
                      type="file"
                      ...
                      className="hidden"
                    />
                    <svg width={40} height={40} viewBox="0 0 24 24" fill="none" stroke={C.sub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" />
                    </svg>
                    <p className="text-sm font-medium mt-4" style={{ color: C.text }}>
                      PDF-Datei hier ablegen oder klicken
                    </p>
                    <p className="text-xs mt-1.5" style={{ color: C.dim }}>
                      Maximal 10 MB, nur PDF-Dateien
                    </p>
                  </div>
                ) : pdfParsing ? (
                  <div
                  ...
                  >
                    <div className="w-8 h-8 rounded-full border-2 border-t-transparent animate-spin mb-4" style={{ borderColor: `${C.accent} transparent ${C.accent} ${C.accent}` }} />
                    <p className="text-sm font-medium" style={{ color: C.text }}>PDF wird gelesen...</p>
                    <p className="text-xs mt-1" style={{ color: C.sub }}>{pdfFileName}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    ...
                    <div
                      ...
                      style={{ background: C.surface, border: `1px solid ${C.greenBorder}` }}
                    >
                      <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: C.greenDim }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate" style={{ color: C.text }}>{pdfFileName}</p>
                        <p className="text-[11px]" style={{ color: C.green }}>
                          {exposeText.length.toLocaleString("de-DE")} Zeichen extrahiert
                        </p>
                      </div>
                      <button
                        ...
                        style={{ color: C.sub }}
                        onMouseEnter={(e) => { e.currentTarget.style.background = C.surface2; e.currentTarget.style.color = C.red; }}
                    ...
                    >
                      <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: C.dim }}>Vorschau</p>
                      <p className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: C.sub }}>
                        {exposeText.slice(0, 1000)}{exposeText.length > 1000 ? "..." : ""}
                      </p>
                    </div>
                  </div>
                )}
            ...
            {error && (
              <div className="mt-3 flex items-center gap-2">
                <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke={C.red} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button
              ...
              disabled={!canAnalyze || pdfParsing}
